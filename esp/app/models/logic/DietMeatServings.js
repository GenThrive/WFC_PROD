      ...
      if (input) {
        ...
        if (userLang === "ESP") {
            ...
            if (input.noteveryday > 0) {
              if ( desc !== '' ) desc += ', ';
              unit = ( input.noteveryday === 1 ) ? ' persona que come carne' : ' personas que comen carne';
              desc += input.noteveryday + unit + ': no todos los días';
            }
            if (input.onceaday > 0) {
              if ( desc !== '' ) desc += ', ';
              unit = ( input.onceaday === 1 ) ? ' persona que come carne' : ' personas que comen carne';
              desc += input.onceaday + unit + ': una vez al día';
            }
            if (input.twiceaday > 0) {
              if (desc !== '') desc += ', ';
              unit = ( input.twiceaday === 1 ) ? ' persona que come carne' : ' personas que comen carne';
              desc += input.twiceaday + unit + ': dos veces al día';
            }
            if (input.everymeal > 0) {
              if (desc !== '') desc += ', ';
              unit = ( input.everymeal === 1 ) ? ' persona que come carne' : ' personas que comen carne';
              desc += input.everymeal + unit + ': en todas las comidas';
            }
