define(function( require ){
  ...
  if (window.location.href.search('shower-duration') >= 0 && first_time_score === 0) {    
   ...
   jQuery('body').delegate("#jump-question","mouseover",function(event){    
           if ($('#hover_arrow').length > 0) {
               if ($("#hover_arrow").css('display') === 'none') {
               $('#hover_arrow').fadeIn().delay(2000).fadeOut('slow');
              }
           }               
      });
   ...
   */
   ...
   /* piano animation on results page chart */
   /* DOES NOT WORK ON ALL BROWSERS. DISABLED */
   /*
        var do_ = 0; 
    ...
    */
    ...
    $('body').delegate('.reminder',touchClick,function(e){
     ...
     $('body').delegate('#close-email,#lightbox.email',touchClick,function(){
      ...
      $('body').delegate('#jump-questions',touchClick,function(){
         $(window).scrollTop(0);         
      });
    ...
    $('body').delegate('#feedback_text','keydown',function(e){
